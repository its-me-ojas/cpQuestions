...
public:
  int myAtoi(std::string s) {
    int i = 0;    // Initialize a pointer to the beginning of the string.
    int sign = 1; // Initialize the sign of the number to positive.
    long long result =
        0; // Initialize the result as a long long to handle overflow.
    ...
    if (i < s.length() && (s[i] == '+' || s[i] == '-')) {
      sign = (s[i] == '-') ? -1 : 1; // Set the sign accordingly.
      i++;
    }

    // Parse the digits while they are valid.
    while (i < s.length() && isdigit(s[i])) {
      result = result * 10 +
               (s[i] - '0'); // Convert the digit character to an integer.
    ...
    // Multiply the result by the sign and cast it to an integer.
