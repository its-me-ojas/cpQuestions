...
// [3,4], becomes the second row in the constructed 2D array. Example 2:
...
public:
  vector<vector<int>> construct2DArray(vector<int> &original, int m, int n) {
    ...
    // to check if its even possible to convert the original vector ito the 2d
    // vector in given conditions
    if (originalSize != (m * n))
      return vector<vector<int>>();
    vector<vector<int>> newVector;
    int index = 0;
    for (size_t i = 0; i < m; i++) {
      // we use temp here to make the rows
      vector<int> temp;
      for (size_t j = 0; j < n; j++) {
        // push the row wise elements into the temp vector
        temp.push_back(original[index]);
        index++;
      }
      // finally push the temp ( new row ) into the 2d vector
